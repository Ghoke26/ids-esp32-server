"""
IDS ESP32 - Server Python (Versi Railway)
"""
...
<body>
  ...
  <div class="stats">
    <div class="stat"><div class="stat-label">Total Serangan</div><div class="stat-val merah">{{ total }}</div></div>
    <div class="stat"><div class="stat-label">IP Unik</div><div class="stat-val kuning">{{ ip_unik }}</div></div>
    <div class="stat"><div class="stat-label">Port Terbanyak</div><div class="stat-val hijau">{{ port_terbanyak }}</div></div>
    <div class="stat"><div class="stat-label">Serangan Terakhir</div><div class="stat-val" style="font-size:14px;padding-top:6px">{{ terakhir }}</div></div>
  </div>
  ...
  {% else %}
  <div class="empty">Belum ada serangan terdeteksi.<br>Pastikan ESP32 sudah update SERVER_URL ke URL Railway.</div>
  {% endif %}
</body>
</html>
"""
...
def dashboard():
    ...
    port_terbanyak = Counter(l['protokol'] for l in log_serangan).most_common(1)[0][0] if log_serangan else "-"
    logs = list(reversed(log_serangan[-50:]))
    return render_template_string(DASHBOARD_HTML, logs=logs, total=total, ip_unik=ip_unik, port_terbanyak=port_terbanyak, terakhir=terakhir)

@app.route('/api/logs')
def api_logs():
    return jsonify({"total": len(log_serangan), "logs": list(reversed(log_serangan[-20:]))})
